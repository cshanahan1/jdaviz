...
def test_cubeviz_aperphot_unit_conversions(cubeviz_helper,
    ...
    cube with units of Jy / pix2 and a cube with units of Jy / sr.

    The aperture photometry plugin should respect the choice of flux and angle
    unit selectedin the Unit Conversion plugin, and inputs and results should
    be converted.
    """
    ...
    # test aperture photometry with all possible flux unit conversions
    for new_flux_unit in SPEC_PHOTON_FLUX_DENSITY_UNITS:
        ...
        if new_flux_unit != flux_unit_str:  # dont compare same units
            ...
            # reset to 'original' unit so were not comparing to the last iteration
            uc.flux_unit.selected = flux_unit_str
            ap.background_selected == 'Manual'
            ap.background_value = 1.
            ap.flux_scaling = 1.

            # then set to new unit
            uc.flux_unit.selected = new_flux_unit

            # make sure display units in aperture phot plugin reflect change
            # multiply out angle to avoid formatting inconsistencies
            assert (u.Unit(ap.display_unit) * angle_unit) == (u.Unit(new_flux_unit))
            assert ap.flux_scaling_display_unit == new_flux_unit

            # make sure background and flux scaling were converted to new unit
            assert_allclose((ap.background_value * new_un).to(flux_unit, equiv).value, 1.)
            assert_allclose((ap.flux_scaling * new_un).to(flux_unit, equiv).value, 1.)
            ...
            _compare_table_units(orig_tab, new_tab, orig_flux_unit=flux_unit,
                                 new_flux_unit=u.Unit(new_flux_unit),
                                 equivalencies=equiv)
